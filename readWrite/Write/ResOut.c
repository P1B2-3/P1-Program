...
void write(Exam_block_t *****schema_data, FILE *fptr) {
    int i,k,j,m,n;
        ...
        for(j = 0; j < DAY_SIZE; j++)
        {
            ...
            for(m = 0; m < ROOM_SIZE; m++)
            {
                ...
                for(i = 0; i < EXAM_SIZE; i++)
                {
                    ...
                    fprintf(fptr, "Exam name: %s",schema_data[0][k][j][m][i].subjects.subjects);
                    fprintf(fptr, "Class: %c.%i\n",schema_data[0][k][j][m][i].classname,schema_data[0][k][j][m][i].year);
                    fprintf(fptr, "Student id: %i\n",schema_data[0][k][j][m][i].students.unikID);
                    fprintf(fptr, "Student name: %s %s\n\n",schema_data[0][k][j][m][i].students.name,schema_data[0][k][j][m][i].students.surname);
